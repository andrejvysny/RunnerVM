...
/// `instances/<uuid>/worker.lock` (and, for a build, `builds/<id>/vm/worker.lock`). vmworker takes
/// an `F_WRLCK` on it before building any VZ object and the kernel drops it when that process dies,
/// which makes it the one liveness signal runnerd can trust — a PID can be recycled, a lock cannot.
///
/// Public so callers outside `ImageStore` (the Phase 5 build supervisor) can probe a build's lock
/// the same way `InstanceStore`/`BuildStore` do internally.
public enum WorkerLock {
  ...
  public static func holderPID(at url: URL) throws -> pid_t? {
  ...
  public static func requireUnheld(at url: URL) throws {
