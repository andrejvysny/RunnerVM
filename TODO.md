...
- [ ] **Security gap, reusable lifecycle**: `agent.cleanup` (`GuestAgent/internal/cleanup/cleanup.go`) only sweeps an allowlist of cache dirs — `DefaultHomeRelPaths = [".cache", ".npm", ".yarn/cache", ".nuget/packages", ".gradle/caches"]` (`cleanup.go:30`), plus `_work`/`_diag` and runner-owned temp entries. It never touches credential files a job's own build tooling might write outside that allowlist: `~/.gitconfig`, `~/.netrc`, `~/.npmrc`, `~/.cargo/credentials.toml`, `~/.docker/config.json`, `~/.ssh/*`. `agent.cleanup` reports `{ok: true}` regardless — no error, silent gap. The escape hatch (`Service.Config.CleanupExtraPaths`, `service.go:72-73,189`) has no CLI flag in `main.go` and nothing in `packaging/launchd`, `packaging/systemd`, or `install.sh` sets it — as shipped, no image protects these paths, not even opt-in. Consequence: on a `reusable`-lifecycle VM, if job A's build step writes a persisted credential (e.g. the common `git config --global url."https://x-access-token:$TOKEN@…".insteadOf …` pattern for private-dependency auth — this exact pattern caused a real incident on an unrelated, manually-managed runner: a stale token in `~/.gitconfig` silently shadowed every later job's own fresh token), it survives cleanup and is inherited by job B on the same reused VM — cross-job credential leakage, not just a reliability bug. `ephemeral` lifecycle (the recommended default) is unaffected: the whole instance disk is destroyed after every job. Fix: expand the default home sweep (or add a credential-specific one) to cover these paths, wire `CleanupExtraPaths` to an actual flag/config, and add a test asserting a reusable cleanup pass strips known credential files.
